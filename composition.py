...
def expose(query_func):
  ...
  for r in range(0, rows):
    ...
    # making many queries.
    vals = [many_results[i][r][-1] for i in range(num_iterations)]
    # Compute the average value (which should tend toward the non-noisy value)
    value = sum(vals) / num_iterations
